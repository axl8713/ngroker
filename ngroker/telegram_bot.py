...
class Telegram_bot(object):
    ...
    _predefined_manager_tunnel_data = {
        "protocol": 'http', "port": 4040, "name": "ngrok manager"}
    _predefined_ssh_tunnel_data = {
        "protocol": "tcp", "port": 22, "name": "SSH"}
    _predefined_deluge_webui_data = {
        "protocol": "http", "port": 8112, "name": "deluge webui"}
    ...
    def __init__(self):
        ...
        MessageLoop(self._bot, {'chat': self.handle_message,
    ...
    def handle_start_command(self, chat_id):
        ...
        self._bot.sendMessage(chat_id, 'Select a command',
            ...
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    ...
                    InlineKeyboardButton(
                        text=self._predefined_ssh_tunnel_data["name"], callback_data=self._callback_data_ssh),
                    InlineKeyboardButton(
                        text=self._predefined_manager_tunnel_data["name"], callback_data=self._callback_data_manager),
                    InlineKeyboardButton(
                        text=self._predefined_deluge_webui_data["name"], callback_data=self._callback_data_deluge)
                ],
            ])
            self._bot.sendMessage(
                chat_id, 'Please select the tunnel to be opened.', reply_markup=keyboard)
        else:
            self.send_please_start_ngrok_message(chat_id)

    def send_please_start_ngrok_message(self, chat_id):
        self._bot.sendMessage(
            chat_id, 'Please start client first via ' + self._bot_command_start + ' command.')
    ...
    def handle_new_tunnel_command(self, chat_id):
        if self._ngrok_client.is_ngrok_started:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text='TCP', callback_data=self._callback_data_tcp),
                    InlineKeyboardButton(
                        text='HTTP', callback_data=self._callback_data_http),
                ],
            ])
            self._bot.sendMessage(
                chat_id, 'Please select a protocol for the tunnel.', reply_markup=keyboard)
        else:
            self.send_please_start_ngrok_message(chat_id)
    ...
    def handle_stop_command(self, chat_id):
        ...
        self._bot.sendMessage(chat_id, 'Select ' + self._bot_command_start +
                              ' command to start client', reply_markup=keyboard)
    ...
    def on_callback_query(self, message):
        ...
        try:
            ...
            self.open_predefined_tunnel(
                self._predefined_tunnels[query_data], query_id, from_id)
        except Exception as ex:
    ...
    def open_predefined_tunnel(self, tunnel_data, query_id, from_id):
        self._bot.answerCallbackQuery(
            query_id, text='Opening ' + tunnel_data["name"] + ' tunnel. Stay tight.')
        tunnel = self._ngrok_client.open_tunnel(
            tunnel_data["protocol"], tunnel_data["port"], tunnel_data["name"])
